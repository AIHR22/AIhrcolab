...
async function seedDatabase() {
  ...
  try {
    ...
    await supabase.from("time_off_requests").delete().neq("id", "0")
    await supabase.from("reviews").delete().neq("id", "0")
    await supabase.from("projects").delete().neq("id", "0")
    await supabase.from("employees").delete().neq("id", "0")
    await supabase.from("company_settings").delete().neq("id", "0")
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const startDate = format(subDays(new Date(), Math.floor(Math.random() * 365 * 2)), "yyyy-MM-dd")
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const startDate = format(addDays(new Date(), Math.floor(Math.random() * 30)), "yyyy-MM-dd")
      const endDate = format(addDays(new Date(startDate), Math.floor(Math.random() * 7) + 1), "yyyy-MM-dd")
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const scheduledDate = format(addDays(new Date(), Math.floor(Math.random() * 30)), "yyyy-MM-dd")
    ...
    for (let i = 0; i < projectNames.length; i++) {
      const startDate = format(subDays(new Date(), Math.floor(Math.random() * 90)), "yyyy-MM-dd")
      const endDate =
        Math.random() > 0.3
          ? format(addDays(new Date(startDate), 30 + Math.floor(Math.random() * 60)), "yyyy-MM-dd")
          : null
